...
6. **Regenerate the paper (and spec) figures:** The figures in `docs/assets/paper/` are *generated* from the live renderer by `scripts/paper_figures.py` (each one is built by wrapping a real `render()` call or a gallery SVG with annotation chrome — see [[paper-figure-house-style]] in the house style). They are **not** hand-drawn and must never be edited by hand. After any change that can alter rendered output (the same trigger as the gallery step) **or** after bumping `SPEC_VERSION`, re-run `PYTHONPATH=src .venv/bin/python scripts/paper_figures.py` (deps: the `render` group) and commit the updated `*.svg` + `*.png`. This is enforced: `tests/test_paper_figures.py` regenerates every figure's SVG in-process and fails CI if it drifts from the committed file, and also fails if a figure's embedded `data-spec-version` no longer matches the current `SPEC_VERSION`. When that test fires on a spec bump, also **eyeball the hardcoded annotation labels** in the generator (e.g. "fingerprint bit *i* fills box *i*") against the new spec — the byte-diff catches pixel drift, but only a human catches a caption whose *meaning* went stale. (The pre-v6 PNGs still in `docs/assets/` are referenced only by `docs/spec.md`; they are *not* generated and are slated for the same treatment — see the spec-figure audit.)
