...
7. **Regenerate the repo self-image (README + social card):** The repo's own "logo" — `docs/assets/root-commit-entviz.svg` (shown in `README.md` and atop the docs) and the GitHub social-preview card `docs/assets/social-card.{svg,png}` — embeds an entviz of this repo's root commit SHA (the tool applied to itself), so it goes stale on the same trigger as the gallery/figures. After any output-altering change, run `uv run python scripts/social_card.py` (it renders the mark in-process via the library — no `entviz` console script on PATH required; `cairosvg` is still needed for the PNG step, via the `render` group) and commit the three regenerated files. **This is now guarded** (since 2026-06-19): `tests/test_social_card.py` regenerates `root-commit-entviz.svg` and `social-card.svg` in-process and fails CI if either drifts (the PNG is checked only for canvas size and the 1 MB cap, since raster bytes aren't stable across cairosvg/font versions). `scripts/release.py` regenerates the card too (since 2026-08-21), on the same footing as the gallery, and runs the suite *after* the bump so a release is tested against the tree it actually tags. Before that fix the card was the release's blind spot twice: v0.9.1 shipped old art *before* the guard existed, and v0.18.2 failed to ship at all *after* it — the tag carried a card still reading `lib="0.18.1"`, release.yml's test gate caught it, and no PyPI artifact or GitHub Release was ever produced. You still regenerate the card yourself for any ordinary output-altering change; release.py only covers the version stamp at release time. The `social-card.png` must *additionally* be re-uploaded by hand under repo **Settings → Social preview** (see README); regenerating the file does not update the GitHub setting.
