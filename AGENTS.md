...
7. **Regenerate the repo self-image (README + social card):** The repo's own "logo" — `docs/assets/root-commit-entviz.svg` (shown in `README.md` and atop the docs) and the GitHub social-preview card `docs/assets/social-card.{svg,png}` — embeds an entviz of this repo's root commit SHA (the tool applied to itself), so it goes stale on the same trigger as the gallery/figures. After any output-altering change, run `uv run python scripts/social_card.py` (it shells out to the `entviz` CLI, so that console script must be on PATH — bare `python scripts/social_card.py` fails with `FileNotFoundError: 'entviz'`) and commit the three regenerated files. **Nothing guards this:** there is no test or figure-drift check over the social card, and `scripts/release.py` does **not** regenerate it (it only re-stamps the gallery's version), so a release can silently ship a stale self-image — which is how v0.9.1 went out with old art. The `social-card.png` must *additionally* be re-uploaded by hand under repo **Settings → Social preview** (see README); regenerating the file does not update the GitHub setting.
