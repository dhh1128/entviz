...
def fig_example():
    """A clean, representative entviz — the spec's opening illustration."""
    art = gallery("11-256-bit-hex.svg")
    margin, top = 40, 40
    art_w = 340
    cap = ("A representative entviz of a 256-bit input: text grid, color bar, surround texture, "
...
def fig_crc():
    ...
    if maxs:
        ...
        rows.append((mp(*marker_center(c)),
                     "blank-cell map — max (plus)", "the cell holding the largest fingerprint token",
                     c.get("stroke") or INK))
    if mins:
        c = mins[0]
        rows.append((mp(*marker_center(c)),
                     "blank-cell map — min (dot)", "the cell holding the smallest token",
                     c.get("fill") or INK))
    if polys:
        p = polys[0]
