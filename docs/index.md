...
1. Convert the *quant* for T into an RGB value the same way CSS does it &mdash; red in the low-order byte, and so forth &mdash; and call this RGB value the **nucleus background color**. The **foreground color** is white (#ffffff) or black (#000000), whichever yields higher WCAG luminance contrast against the *nucleus background color*. Equivalently: if the bg's relative luminance is below the WCAG black-vs-white crossover at `Y ≈ 0.179` (which is the value where `contrast(white, bg) = contrast(bg, black)`), use white; otherwise use black. The naive `Y < 0.5` rule used in v3 mis-paired medium-luminance backgrounds (e.g., light beige #c3b2a1 at Y ≈ 0.47) with white, producing WCAG ratios of 2-3:1 that fail AA; the 0.179 crossover always yields the higher-contrast pairing.
