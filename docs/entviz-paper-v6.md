...
Humans are poorly built for the question as usually posed. Comparing `fc94b0c1e5b0987c` with `fc94b0c1e5b0987d` forces a serial, character-by-character scan — slow, effortful, and easy to abandon one digit early [3]. Recent work quantifies the cost directly: in a 162-participant study, Turner, Shahandashti, and Petrie found that comparison errors persist even when fingerprints are chunked for inspection, and that error rates climb with fingerprint length, which led them to conclude that the 60-digit safety numbers shipped by Signal and WhatsApp deliver less security in practice than their bit length suggests [7]. The failure is one of system design, not user diligence — the case Whitten and Tygar made when they showed that most security failures trace to interfaces that ignore how people actually work, not to careless users [31]. The task demands a cognitive mode the brain runs badly — serial symbol matching — while ignoring one it runs superbly: fast, parallel, holistic pattern recognition [4]. When the comparison fails, the consequences are a man-in-the-middle compromise, a misdirected payment, or an erosion of trust in the tool that was supposed to help.

## 1.2. Hash Visualization as a Human-Centric Solution

Hash visualization, introduced by Perrig and Song in 1999, attacks the bottleneck by changing the modality of the task [2]. The idea seeded a lineage of human-centric authentication visualizations, from image-recognition schemes [5] to the SSH randomart now generated by default in OpenSSH. Instead of comparing strings, the user compares pictures. The aim is to trade a known human weakness (serial string comparison) for a known human strength (holistic pattern recognition) so that a difference between two values becomes not merely detectable but immediately obvious. The shift is from slow analytical judgment to fast perceptual judgment. Perrig and Song framed the security requirement precisely: a useful hash visualization must resist *near-collisions*, meaning it must be hard to find two distinct inputs that map to perceptually similar images [2].
...
The JND is the smallest change in a stimulus an observer detects at least half the time [14]. Weber's law states that, over much of each sensory range, the JND is a roughly constant fraction of the baseline intensity: ΔI / I = k, with k the Weber fraction. Discrimination is relative, not absolute — a fixed increment is easier to notice against a small baseline than a large one [15].

### 2.2.2. JND Thresholds Mapped to Entviz Features

The original paper tabulated JND thresholds for generic visual primitives. Because v6 uses specific primitives, we map each threshold to the channel it governs. The point of the table is not the raw numbers — those are stable, cited results — but the assignment in the rightmost column, which says which design decision each threshold constrains.
...
Gold's position is a true optimum, not a guess. Holding white, red, blue, and black fixed and moving only gold, the two adjacent gaps — white→gold and gold→red — are equalized (the **maximin**) at L\* ≈ 78.5, giving each a gap of ΔL\* ≈ 21. This corrected a real defect: v5's gold `#ffd966` sat at L\* ≈ 88, making the white→gold gap only ΔL\* ≈ 12 — the weakest pair in the palette, and nearly invisible on a grayscale or achromatopsia rendering. Gold/red can accept the equal-and-no-larger gap because it carries a hue cue (yellow vs. red) as backup; white/gold has no hue backup that survives CVD, so the lightness budget is spent equalizing it.

We are explicit about a metric trap here, because it generalizes. An obvious approach is to maximize pairwise **ΔE76** (Euclidean distance in CIELAB) under CVD simulation. That is the wrong target: ΔE76 rewards chroma, and chroma is exactly the channel that collapses under CVD. Optimizing ΔE76 optimizes the *least* robust dimension; a higher ΔE76 is not a safer palette. We discarded a ΔE76-"optimized" palette that looked worse to every observer and optimize on ΔL\* instead [18, 20].

The honest limit is stated rather than hidden. Lightness spacing is robust but not CVD-proof. Under **protanopia**, simulation darkens red until its lightness approaches blue's — the red/blue pair collapses to ΔL\* ≈ 7 *for any choice of gold*, because we are not moving red or blue. No palette over these five hues removes it. Table 4 records the worst-case pairs.
...
Until these are settled, the right characterization of entviz is *theoretically motivated and internally adversarially reviewed, but empirically untested* — and the comparison to randomart should be read as "clears the best published, preliminary estimate of randomart's perceptual entropy," not as a measured human result.
