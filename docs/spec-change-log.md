...
The finding came from a Claude Security scan of entviz-js (2026-07-31, F4), which reported it as a port bug. It was not — the python reference behaved the same way, so the spec was what needed deciding. See `this.i:hrpb1nd`.

1. **The HRP binds by prefix-fold.** `<hrp>1` is kept as the prefix and the fingerprint hash input becomes `<hrp>1 ‖ core` — the mechanism `did:<method>:`, `urn:<nid>:`, SWHID and gitoid already use. This follows from the spec's own **swap test** (`this.i:s3mpr3fx`): substituting one HRP for another leaves every data character untouched — only the 6-character checksum, which is the *suffix*, is recomputed — and the value denotes something different, which is the identity branch. The fold rather than the in-core mechanism is forced by the alphabet: bech32's charset excludes `b`, `i`, `o` and `1`, so an HRP cannot live in a BECH32 core. Applies to the generic `<hrp>1<data>` form, Bitcoin segwit `bc1`/`tb1`, Litecoin `ltc1`, and Cardano Shelley `addr1`/`stake1`/`addr_test1`.
2. **The bech32 checksum is the suffix on every path.** The `bc1`/`tb1` and `ltc1` parsers previously left the 6-character checksum *inside* the core. That inflated `size_bits` and — because the polymod covers the HRP — bound the HRP by accident, which is why those two schemes never exhibited the collision while the generic and Cardano paths did. v16 moves the checksum to the suffix where every other verified checksum lives (`this.i:sufxbind`) and replaces the accident with the explicit fold. The two changes are one change: moving the checksum out on its own would have *created* a `bc1`/`tb1` collision.
3. **CashAddr is a deliberate exception.** Bitcoin Cash keeps its 8-character checksum in the core and is not folded, because its prefix is *optional* — a bare `q…`/`p…` body means `bitcoincash`. Folding the literal prefix would make one address and its own prefixed spelling render differently; folding a synthesized canonical prefix would put text into `parts` the input never contained. It is safe unfolded precisely because the in-core checksum covers the prefix.
4. **A folded prefix that is not the PRIMARY slot must still be labeled.** v15 omitted folded prefixes from the trailing `PREFIX` slot on the grounds that they are already the PRIMARY slot — true for `did:key`/`urn:isbn`/`swh:1:cnt`. A bech32 HRP's PRIMARY is the scheme name (`bech32`, `BTC`, `ADA`), so the same rule would have deleted the HRP from the label entirely, leaving it in no text channel at all. The rule is now exact: drop the slot only when the prefix, minus a trailing `:`, equals the PRIMARY string. Labels such as `bech32, cosmos1` and `BTC, bc1` are unchanged from v15; what changed is that they are now guaranteed.
5. **The text channel's lossless claim is qualified.** The spec claimed that reading the text aloud transfers all information. That was already false for every folded prefix — the cells carry the core alone, so two DIDs differing only in method have identical cell text. v16 states the exception where the claim is made and **requires** any read-aloud, comparison-text, or guided-comparison procedure to include the label strip's prefix rather than affirming sameness from cell text alone.
6. **Six new corpus vectors, each an HRP sibling of an existing one.** `osmo`, `btc-segwit-testnet`, `cardano-shelley`, `cardano-shelley-testnet`, `nostr-npub`, `nostr-nsec`. Each shares its payload with a neighbour and differs only in the HRP, so any implementation that reproduces both goldens has necessarily bound the HRP. They are ordinary render vectors rather than a new distinctness-pair concept precisely because the goldens now differ.

Four golden rasters change (`btc-segwit`, `btc-segwit-p2wsh`, `cosmos`, `litecoin`); `bitcoincash` does not, which is the exception behaving as specified.

**Ports.** This propagates to the four sister-repo ports (entviz-js, -rs, -java, -go): fold the HRP on all four bech32 paths, move the segwit/Litecoin checksum to the suffix, leave CashAddr alone, fix the label rule, and reproduce Tier A and Tier B against the regenerated corpus. entviz-js additionally carries the comparison-side half of item 5 (tick `2rf4`).
...
4. **SSH ECDSA curve shortened in the label: `ecdsa-nistp256` → `ecdsa-p256`** (and `-p384`/`-p521`). `p256` is the curve's common short name and there is no rival non-NIST `p256`, so the standards-body word is dropped; the algorithm word stays (a naive reader needs `ecdsa` — `p256` is a curve, not a signature scheme). The `data-qualifiers.algorithm` characterization field keeps the faithful SSH id (`ecdsa-nistp256`); only the projected label shortens it.
