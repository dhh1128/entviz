...
The v4 spec is archived at [v4/index.md](v4/index.md); v3 at [v3/index.md](v3/index.md). v5 keeps v2's fingerprint, v3's color bar skew / frame / ellipse rework / hex font fitting, and almost all of v4 (24-box surround, per-cell edge color, taller nucleus, 3:2 cell aspect, digest-histogram color bar, hybrid ellipse anchoring, label strips, blank-cell ring with clock hands). The substantive v5 changes target the head-and-tail truncation weakness exposed by the 2026-05-27 adversarial review (finding F5) and the CVD-accessibility ask in finding F6.
...
1. Normalize the input.
    ...
    **Case normalization is intentional and load-bearing.** For every case-insensitive alphabet (hex, UUID, base32, bech32, crockford32, and EOS's base alphabet), normalization lowercases (or otherwise canonicalizes) the input before it is fingerprinted. As a result, two inputs differing only in case for a case-insensitive alphabet produce **identical** entvizes. This is required: without it, a benign tooling difference (one system emits uppercase hex, another lowercase) would render as an entropy difference. Cell text shows the *normalized* form, not the input form. (See `this.i:c4s3norm`.)
...
1. Split the entropy string into tokens. Each token represents 3 bytes (24 bits) of binary entropy, or as close to that amount as possible while respecting whole-character boundaries of the underlying encoding. The **token length** (chars per token) is determined by the **alphabet** the parser declared for the input — not by inspecting the content of the core or by string-matching the type name. (Content inspection is unsound: a base32 value, for instance, can use only characters from the hex alphabet and would be indistinguishable from hex on inspection. Each parser knows which alphabet its core uses and must declare it.) The alphabets in this spec are:
    ...
    **Used ftoks** follow the same one-to-one rule as for short inputs: cell index → used ftok index. The two separator blank cells consume their own cell indices (8 and 13), so the used ftoks at those indices are skipped (no token there to consume one). Middle-slice cells use used ftoks at indices 9..12; tail cells use used ftoks at indices 14..21. The fingerprint is still SHA-512 over the entire normalized entropy (unchanged from v4).
...
1. **Blank cells** carry no token. For a blank cell, draw no nucleus, no text, and no surround boxes; the grid_rect's background color shows through. On the **first cell of each run** of consecutive blank cells in reading order, draw a **blank-cell marker**: a white disc with a thin black rim, overlaid by two clock-hands indicating the *minftok* and *maxftok* directions (defined below). Subsequent cells in the same run are truly empty (no ring, no hands). This per-run rule prevents long trailing runs of blanks on larger entropies from producing a forest of redundant markers.
    ...
    * **Long hand → maxftok direction.** Length = `nominal_radius + 1` (extends 1 px past the rim). Rendered as a `<line>` with `stroke = #ffffff`, `stroke-width = 1`, and `style="mix-blend-mode: difference"`. The difference-blend mode is essential: on the white disc interior the white stroke inverts to black, so the hand reads as black against the disc; where the hand crosses the 1-px black rim, it inverts to a single white pixel — a "notch" through the rim that visually marks the angle.
      ...
      *Rendering-environment dependence.* CSS `mix-blend-mode` is well-supported in browsers (Chrome, Firefox, Safari, Edge) but NOT in all SVG renderers. Notably, `cairosvg` (commonly used for server-side PNG generation) silently ignores `mix-blend-mode`, so the long hand renders as a 1-px white line that is invisible against the white disc and disappears against the black rim. This means PNG snapshots produced via cairosvg lose the maxftok direction indicator. Consumers that need lossless rendering in non-browser environments should either (a) use a browser-based renderer (Playwright, Puppeteer, headless Chrome) or (b) post-process the SVG to substitute the difference-blend hand with a directly-stroked dark hand before rasterizing. The short hand (maxftok direction encoded by the small terminator circle) does not depend on `mix-blend-mode` and renders correctly in all SVG renderers.
    * **Short hand → minftok direction.** Length = `nominal_radius / 2` (= 8.5 at 12pt). Rendered as a `<line>` with `stroke = #000000`, `stroke-width = 1`. Terminated by a `<circle>` at the hand's endpoint with `r = 1.5`, `fill = #ffffff`, `stroke = #000000`, `stroke-width = 1` — visually a small circle with a white dot inside. This terminator distinguishes the short hand's tip from the long hand's rim notch.
    ...
    Blank cells include both the up-to-3 algorithm-inserted blanks (median, ASCII-last, ASCII-first) and any trailing unfilled cells; the two large-input separator blanks (between head and middle, and between middle and tail; see the large-input handling subsection) are also blank cells for purposes of this rule. The whole marker stack (ring + clock-hands) is drawn after the ellipse overlay so it sits on top of any overlay tint.
