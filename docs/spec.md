...
The v1–v5 specs are archived in the project's git history (browse the `docs/v1`…`docs/v5` folders, or earlier `docs/index.md`, at the corresponding release commits on [GitHub](https://github.com/dhh1128/entviz)). v6 keeps everything from v5 (head + middle-slice + tail tokenization, the loud `part of` truncation marker, color-bar letters) and v4's structure (24-box surround, per-cell edge color, 3:2 cell aspect, digest-histogram color bar, hybrid ellipse anchoring, label strips). The v6 changes are all in the rendering layer; the tokenization, fingerprint, and text channel are unchanged from v5.

## What's new in v6

v6 makes four rendering refinements:

1. **Wider color bar.** The color bar's width doubles from `box_height` to `bar_width = 2·box_height`, so the per-band color letters (`w`/`g`/`r`/`b`/`k`) render large enough to be legible. The bar width is now a named geometry term rather than a reuse of `box_height`. See the geometry and color-bar steps.
2. **Redesigned blank-cell marker.** v5's white-disc-with-clock-hands marker is replaced. Every blank cell now carries a black-outlined rounded rectangle; the first blank cell additionally becomes a **map** — a miniature scale model of the grid, filled white (or gold on a white-background entviz), with a red dot marking the *maxftok* cell's position and a blue dot marking the *minftok* cell's. This conveys those cells' *positions* (not just directions), and uses no `mix-blend-mode`, so it renders identically in browsers and non-browser rasterizers (closing adversarial finding F-A6 for this channel). See the blank-cell step in the cell rendering algorithm.
3. **Clamped ellipse overlay.** The ellipse's `rx`/`ry` bounds change from `[nucleus_height, d_far − cell_width]` to `[0.22·d_far, 0.58·d_far]`, so the overlay covers a *noticeable but partial* share of the grid on every grid size — never an invisible sliver, never swamping the grid. See the ellipse-overlay step and `reviews/ellipse-audit-2026-06-02.md`.
4. **Refreshed font fallback chain.** The monospace fallback chain is reordered and extended to cover iOS and Android explicitly (not just Linux/Windows/macOS). See the font-family note.

Implementations that supported v5 need only make these four rendering changes to support v6.
...
* Losslessly represent all bits of entropy up to 512 bits. For larger inputs, losslessly represent the first 192 bits and the last 192 bits in the text channel; additionally show 4 fingerprint-selected 24-bit middle slices of the input in the text channel; and bind the entire input through the fingerprint.
...
1. Split the entropy string into tokens. Each token represents 3 bytes (24 bits) of binary entropy, or as close to that amount as possible while respecting whole-character boundaries of the underlying encoding. The **token length** (chars per token) is determined by the **alphabet** the parser declared for the input — not by inspecting the content of the core or by string-matching the type name. (Content inspection is unsound: a base32 value, for instance, can use only characters from the hex alphabet and would be indistinguishable from hex on inspection. Each parser knows which alphabet its core uses and must declare it.) The alphabets in this spec are:
    ...
    * **Middle-slice group (M = 4 tokens).** Sample 4 contiguous 24-bit (3-byte) windows from the body of the input at offsets derived from the fingerprint, as described below. The middle-slice tokens carry cell indices 9..12 (after the first separator at cell index 8), in ascending offset order.
    ...
    * **Separator blanks.** Two blank cells separate the three groups. The first separator occupies cell index 8 (between the head's last cell at index 7 and the first middle slice at index 9). The second separator occupies cell index 13 (between the last middle slice at index 12 and the first tail cell at index 14). The total cell count is `H + 1 + M + 1 + T = 8 + 1 + 4 + 1 + 8 = 22`, which exactly fits the 22-cell budget.

    Choice of H, T, M: the allocation is fixed at H=8, T=8, M=4 with 2 separator blanks. Rationale: this consumes exactly 22 cells (no spare); it keeps the head and tail visibly dominant (each occupying a full 8-token row when the grid is laid out with cols≥8); and it leaves the middle group large enough — 96 bits drawn from up to 4 separate body regions — that a head+tail collision attacker faces a meaningful additional barrier. A 6/6/8 or 10/10/2 split was considered and rejected: 6/6/8 makes the middle visually dominate the head/tail (which are the parts a user is most likely to recognize), and 10/10/2 makes the middle so small that two of the four middle cells would be redundant on most inputs.

    **Middle-slice offset derivation.** Let `input_length_bytes` be the body's byte length (the count of raw bytes that the normalized core decodes to under its declared alphabet — *not* the character count). Let `digest` be the 64-byte SHA-512 fingerprint of the normalized core. For each slice index `i ∈ [0..M−1] = [0..3]`:

    1. Read 2 bytes from `digest[32 + 2·i .. 33 + 2·i]` as a big-endian uint16. Call this `raw_i`.
    2. Compute `offset_i = (raw_i mod (input_length_bytes − 3 − 32)) + 16`. The subtraction reserves 32 bytes at the tail (16 bytes of guard band before the tail-192-bit region plus 16 bytes of the tail region itself) and 3 bytes at the read end (so the read of 3 bytes starting at `offset_i` does not run past the tail-guard line); the `+ 16` shifts the result past the head's 16-byte guard band so offsets fall strictly between the head-32 and tail-32 byte regions.
    3. If `offset_i` is within ±3 bytes of any previously-selected `offset_j` (`j < i`), increment by 3 and retry. If the search wraps once without finding a clean offset (which is exceedingly rare except on inputs only marginally longer than 64 bytes), deterministically fall back to evenly-spaced offsets across the body: `offset_i = 16 + round(i · (input_length_bytes − 32) / M)`.
    4. After all four offsets are derived, sort them ascending. Slices are presented in ascending-offset order (NOT in derivation order), so the user reads the middle group left-to-right through the input.

    **Middle-slice cell text rendering.** Each middle slice reads 3 bytes of the original input starting at its sorted offset, and renders them in the **same alphabet** that the entropy was parsed as. The rendering rule per alphabet:

    * **hex (4 bits/char):** show 6 hex chars (`f"{b0:02x}{b1:02x}{b2:02x}"`).
    * **base64 / base64url / base58 (6 bits/char):** show 4 chars approximating the 3 bytes via that alphabet's standard 3-byte-to-4-char encoding (base64/base64url use the RFC 4648 encoder; base58 uses the standard base58 encoder applied to the 3 input bytes).
    * **base32 / bech32 / crockford32 (5 bits/char):** show 4 chars formed by reading the 24 bits as four 5-bit groups (4 chars cover 20 bits, leaving the low 4 bits unrendered for symmetry with the normal token-length rule). For these alphabets, when the slice offset doesn't fall on an aligned character boundary in the original encoded core, round the offset to the nearest aligned position (whichever character boundary the byte offset lands closest to) and use the bytes at that position; this is the only rounding adjustment the offset receives.
    * **base36 (6 bits/char):** show 4 chars by re-encoding the 3 bytes through base36 the same way base58 is treated.

    The middle-slice cell text is therefore not always a verbatim substring of the input string; it is a verbatim substring of the input's *byte representation*, re-rendered in the input's alphabet. For hex this distinction is invisible (the input is already a byte representation); for the variable-density alphabets it means a middle slice may correspond to a character span that doesn't end exactly on the slice boundary.

    **Used ftoks** follow the same one-to-one rule as for short inputs: the used ftok at *token index i* drives the token with that token index (per the used-ftoks step below), regardless of which *cell* that token lands in. Because the 20 large-input tokens carry token indices 0..19 contiguously while the separator blanks consume cell indices 8 and 13, the mapping from cells to ftoks is: head cells 0..7 use ftoks 0..7; middle-slice cells 9..12 use ftoks 8..11; tail cells 14..21 use ftoks 12..19. The separator blank cells (8 and 13) are not tokens and consume no ftok; ftoks 20 and 21 are unused (only the first *token count* = 20 ftoks are used). The fingerprint is still SHA-512 over the entire normalized entropy (unchanged from v4).

    A T1+T5+T6 attacker (see `threat-model.md`) who could previously collide only the head and tail must now additionally produce matching bytes at four fingerprint-selected middle offsets — and those offsets shift as a function of the full input, so the attacker cannot grind the body's content independently from the offsets. This closes the head-and-tail-only collision pattern that adversarial review finding F5 demonstrated against v4.
...
1. If *token count* is less than *cell count*, the grid will have blank cells. We want to use blank cells to create visual gaps in a consistent way that is more meaningful than simply putting all the blanks at the beginning or end, because this will aid comparison. Each used ftok corresponds to a token (and therefore to a cell); use that correspondence to locate the cells named below. Insert a blank cell at the *cell index* of the token corresponding to the *median ftok* by incrementing the *cell index* of all tokens whose *token index* >= that token's *token index*. This essentially shifts these tokens to the right or down in the grid. If *token count* + 1 is still less than *cell count*, insert a second blank cell before the cell of the last ftok in the ASCII-sorted list, again shifting cells that render after. If *token count* + 2 is still less than *cell count*, insert a third blank cell before the cell of the first ftok in the ASCII-sorted list, again shifting cells that render after. Do not perform more than 3 shifts. (For inputs greater than 512 bits, the two blank cells separating the head, middle-slice, and tail groups are in addition to these — but in practice >512-bit inputs always have *token count* = 20 — equal to the full 22-cell budget minus the two separators — and use a 22-cell grid with the two separators occupying the fixed indices 8 and 13 described in the large-input handling subsection.)
...
1. **Blank cells** carry no token. For a blank cell, draw no nucleus, no text, and no surround boxes; the grid_rect's background color shows through. Every blank cell instead carries a **rounded-corner rectangle** coincident with the cell's *nucleus rect* (the same centered region a nucleus would occupy, `nucleus_width × nucleus_height`), with `fill = none`, `stroke = #000000`, `stroke-width = 1`, and corner radius `font_size_px / 8` (= 2 at 12pt). It is drawn after the ellipse overlay so it sits on top of any overlay tint.
    ...
    * a **blue** circle (`#1d4ed8`) centered in the sub-cell whose (row, column) is that of the **minftok cell**.
    ...
    Each circle's radius is `0.35 × min(sub_cell_width, sub_cell_height)`, where `sub_cell_width = nucleus_width / cols` and `sub_cell_height = nucleus_height / rows`. In the degenerate case where the maxftok and minftok cells coincide (possible only when a single used ftok makes the smallest and largest quant the same cell), draw the blue marker as a stroked ring (`fill = none`, `stroke = #1d4ed8`, `stroke-width = 1`) with the red marker as a filled dot concentric inside it, so both remain visible.
