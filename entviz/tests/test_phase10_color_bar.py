...
def _color_bar_bands(svg):
    """All rects at x=0 with width=GM=4 (the color bar bands)."""
    return [
        r for r in svg.xpath('//*[local-name()="rect"]')
        if float(r.get("x", -1)) == 0 and float(r.get("width", -1)) == 4
    ]
...
def test_color_bar_bands_are_contiguous_top_to_bottom():
    ...
    assert abs(prev_bottom - float(svg.get("height"))) < 0.01
